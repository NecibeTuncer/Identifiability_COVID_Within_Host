...
% clc
% 
%load('Workspace_for_Patient_A.mat')
...
% 
h = histfit(exp(chain(:,1)), 100, 'lognormal');
h(1).FaceColor = [.8 .8 1];
h(2).Color = [.2 .2 .2];
h(2).LineWidth = 1;
xlim([1e-9 1e-7])
...
% %figure(4)
% subplot(3,3,4)
% h = histfit(exp(chain(:,4)), 50, 'lognormal');
% h(1).FaceColor = [.8 .8 1];
% h(2).Color = [.2 .2 .2];
% h(2).LineWidth = 1;
% %L = get(gca,'XLim');
...
%subplot(3,3,5)
subplot(2,2,4)
h = histfit(exp(chain(:,5)), 100, 'lognormal');
h(1).FaceColor = [.8 .8 1];
h(2).Color = [.2 .2 .2];
h(2).LineWidth = 1;
hold on
...
% %figure(8)
% subplot(3,3,8)
% h = histfit(exp(chain(:,8)), 100, 'lognormal');
% h(1).FaceColor = [.8 .8 1];
% h(2).Color = [.2 .2 .2];
% h(2).LineWidth = 1;
% %L = get(gca,'XLim');
...
% plot(xgrid,prior_pdf,'LineWidth',1,'Color',[.5 0 0])
% %ylabel('Probability Density')
% legend('K','Prior pdf','Location','best')
% %hold off
% set(gca,'FontSize',12,'FontName','Arial','linewidth',0.5,'FontWeight','normal')
% xlabel('K','FontSize',18,'FontName','Arial','FontWeight','normal');
% 
% %h = get(0,'children');
% 
% % for i=1:length(h)
